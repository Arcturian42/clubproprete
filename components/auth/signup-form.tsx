...
import { ArrowRight, Building2, CheckCircle2, Eye, EyeOff, GraduationCap, Package, Sparkles, UserRound } from "lucide-react";
...
// F2 — Liste de contrôle des exigences de mot de passe, affichée en direct sous
// le champ pour que l'utilisateur sache exactement ce qui manque.
const passwordRules = [
...
function PasswordRequirements({ password }: { password: string }) {
  return (
    <ul className="mt-2 grid gap-1 sm:grid-cols-2" aria-label="Critères du mot de passe">
      {passwordRules.map((rule) => {
        const met = rule.test(password);
        return (
          <li
            key={rule.label}
            className={`flex items-center gap-1.5 text-xs font-semibold normal-case ${met ? "text-emerald-600" : "text-slate-500"}`}
          >
...
export function SignupForm() {
  ...
  // F1 — Un seul consentement couvrant CGU + politique de confidentialité.
  ...
  function validate() {
    ...
    if (!password) {
      ...
      nextErrors.pwdError = "Le mot de passe ne respecte pas tous les critères ci-dessous.";
    }
    if (!consentAccepted) nextErrors.consent = "Vous devez accepter les conditions et la politique de confidentialité.";
  ...
  async function submitSignup() {
    // U2 — La validation s'exécute à la soumission et affiche immédiatement les
    // erreurs au niveau des champs (plus de bouton désactivé silencieux).
    if (!validate()) return;
    setLoading(true);
    ...
    const result = await registerUser({
      ...
      lastName,
      phone,
      // F1 — Le consentement unique alimente les deux champs légaux côté serveur.
      termsAccepted: consentAccepted as true,
      privacyAccepted: consentAccepted as true,
    });
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      {/* M1 — Sur mobile, le formulaire passe en premier (order-1) ; le bloc
          promotionnel passe dessous (order-2). En lg, on revient à l'ordre visuel. */}
      <div className="surface order-2 p-6 lg:order-1">
        ...
        <div className="mt-4 rounded-[16px] border-2 border-indigo-200 bg-indigo-50 p-4">
          <p className="flex items-center gap-2 text-sm font-bold text-slate-700">
            ...
            Après l&apos;inscription, quelques questions personnalisent votre espace.
          </p>
        </div>
      ...
      {/* U1 — Vrai <form> sémantique : touche Entrée, gestionnaires de mots de
          passe et claviers mobiles "Aller" fonctionnent. */}
      <form
        className="surface order-1 p-6 lg:order-2"
        onSubmit={(event) => {
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className={labelClass}>
            ...
            />
            {errors.firstName && <span id="error-firstName" className="text-xs font-bold text-red-500 normal-case">{errors.firstName}</span>}
          </label>
          <label className={labelClass}>
            Nom *
            <input
            ...
            />
            {errors.lastName && <span id="error-lastName" className="text-xs font-bold text-red-500 normal-case">{errors.lastName}</span>}
          </label>
          <label className={labelClass}>
            Email *
            <input
            ...
            />
            {errors.email && <span id="error-email" className="text-xs font-bold text-red-500 normal-case">{errors.email}</span>}
          </label>
          <label className={labelClass}>
            Mot de passe *
            ...
            <div className="relative">
              <input
                ...
                className={`bento-input w-full pr-11 ${errors.pwdError ? "border-red-500 shadow-[2px_2px_0_#ef4444]" : ""}`}
                value={password}
            ...
            {errors.pwdError && <span id="error-password" className="text-xs font-bold text-red-500 normal-case">{errors.pwdError}</span>}
        ...
        {/* F1 — Consentement unique CGU + confidentialité, liens conservés. */}
        <label className="mt-5 flex items-start gap-3 text-sm font-semibold text-slate-700">
          <input
            checked={consentAccepted}
            className="mt-1 h-5 w-5 accent-indigo-600"
            type="checkbox"
            onChange={(event) => { setConsentAccepted(event.target.checked); setErrors((e) => ({ ...e, consent: "" })); }}
          ...
          <span>
            ...
            .
            {errors.consent && <span id="error-consent" className="block text-xs font-bold text-red-500 normal-case mt-1">{errors.consent}</span>}
          </span>
        </label>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          {/* U2 — Le bouton n'est désactivé que pendant l'envoi (loading), jamais
              en fonction de la validation : un clic affiche toujours un retour. */}
          <button
            type="submit"
